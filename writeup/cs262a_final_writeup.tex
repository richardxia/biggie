...
%(large insertion, deletion, transposition, etc).
...
%that we can perform more thorough
%analyses on the areas of high complexity to obtain more accurate results.
...
%As part of our project, we plan to better define high and low complexity and to provide heuristics for defining each.
...
% along  were the number of reads aligned in each
%direction (forward and reverse complement); the number of reads with a
%substitution, insertion, or deletion; the number of reads that aligned with a
%low mapping quality (possibly indicating a repeat or more complex variant); and
%the number of reads mapping to multiple locations.  See Table~\ref{complexity}
%for relative weights of each scoring metric. 
...
\bibitem{telescoper} Ma'ayan Bresler M., Sheehan S., Chan A., and Song, Y., ``Telescoper: {\it de novo} assembly of highly repetitive regions.'' {\em Bioinformatics} (2012), 28: 311-317.
