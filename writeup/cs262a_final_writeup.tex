...
%oreover, current app and current algorithms, in particular structural variant
...
%ore specifically we begin with the problem of calling single-nucleotide
...
%\subsection{BA Sorting and Indexing}
%The most-common format used for representing an aligned set of reads is the BA (Binary Sequence Alignment ap) format.
%ost sequence aligners will output a BA file but the reads are usually randomly ordered.
...
%
%Although there already exist several tools which can sort and index BA files (\cite{bamtools}, \cite{samtools}), they are limited by the amount of memory and may either fail or resort to paging to disk when out of memory.
%To our knowledge there do not exist any distributed BA file sorters or indexers, and sorting BA files can be one of the slowest steps of the analysis process.
%Although ideally the BA file would only be sorted once for each genome, in practice different alignment software (with different parameters) might be used on the same data multiple times.
%We plan to divide up the unsorted BA file and sort the segments in parallel across a cluster of machines.
%This allows us to keep the BA files in memory and avoid writing to disk.
...
%	\begin{tabular}{|l|c|c|}
%  \hline
%	Algorithm & \# SNPs & \# endelian conflicts  \\
%	\hline
%	CASAVA & 128K & 162 \\
%	\hline
%	Our results & 120K & 84 \\
...
%easuring accuracy will be the most difficult part of the evaluation, since it is the least standardized metric.
...
%endelian conflicts of CASAVA \cite{casava} with our SNP caller on real data, using the
%Yoruban trio to determine endelian conflicts.  
...
\begin{figure}[h!]
  ...
  % TODO: Move following lines to actual discussion
...
\begin{figure}[h!]
  ...
  For each algorithm, a very small percentage of correct SNP bases actually have the incorrect (unphased) genotype.} %These numbers using aligner BWA are GATK: 0.48\%, mpileup: 0.25\%, and BIGGIE: 0.35\%}
	\end{figure}
...
%SAtools is being replaced by GATK as the industry standard.
...
\bibitem{telescoper} Bresler ., Sheehan S., Chan A., and Song, Y., ``Telescoper: {\it de novo} assembly of highly repetitive regions.'' {\em Bioinformatics} (2012), 28: 311-317.

\bibitem{bamtools} Barnett D., Garrison E., Quinlan A., Str\"{o}mber ., and arth G., ``BamTools: a C++ API and toolkit for analyzing and managing BA files.'' {\em Bioinformatics} (2011), 27: 1691-1692.

\bibitem{casava} CASAVA. (2012), http://www.illumina.com/software/genome\_analyzer\_software.ilmn.

\bibitem{kristalcurtis} Curtis, K., personal communication, (2012).

\bibitem{gatk} DePristo ., Banks E., Poplin R., Garimella K., aguire J., Hartl C., Philippakis A., del Angel G., Rivas ., Hanna ., cKenna A., Fennell T., Kernytsky A., Sivachenko A., Cibulskis K., Gabriel S., Altshuler D., and Daly ., ``A framework for variation discovery and genotyping using next-generation DNA sequencing data.'' {\em Nature Genetics} (2011), 43:491-498.
...
\bibitem{samtools} Li H., Handsaker B., Wysoker A., Fennell T., Ruan J., Homer N., arth G., Abecasis G., Durbin R. and 1000 Genome Project Data Processing Subgroup, ``The Sequence alignment/map (SA) format and SAtools.'' {\em Bioinformatics} (2009), 25: 2078-9.

\bibitem{tvsim} Liptrap J., TVsim genomic read simulation program. (2012).

\bibitem{simNGS} assingham T., ``simNGS and simLibrary -- Software for Simulating Next-Gen Sequencing Data.'' (2010), http://www.ebi.ac.uk/goldman-srv/simNGS/.
...
\bibitem{picard} Picard. (2012), http://picard.sourceforge.net/.

\bibitem{platypus} Rimmer A., athieson I., Lunter G., and cVean G., ``Platypus: An Integrated Variant Caller.'' (2012), www.well.ox.ac.uk/platypus.

\bibitem{schuster} Schuster, S. ``Next-generation sequencing transforms today's biology.'' {\em Nature ethods} (2008), 5: 16-18.

\bibitem{structuralVar} Tuzun E. {\em et al}, ``Fine-scale structural variation of the human genome.'' {\em Nature Genetics} (2005), 37: 727-732. 

\bibitem{snap} Zaharia ., Bolosky W., Curtis K., Fox A., Patterson P., Shenker S., Stoica I., Karp R., Sittler T., ``Faster and ore Accurate Sequence Alignment with SNAP.'' (2011) http://arxiv.org/abs/1111.5572.
